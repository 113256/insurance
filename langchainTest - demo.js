...
export const run = async (filePath) => {
	...
	//splitPages = false means 1 doc per pdf, true means 1 doc per page
  ...
  if (cost <= 1) {
    ...
    var res = await chain.call({ query: question });
    console.log({ res });
	res = await chain.call({ query: "Whats the total revenue" });
    console.log({ res });
  } else {
...
run("uploads\\NASDAQ_AMAT_2022.txt");